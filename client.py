...
def download_file(bucket_name, file_name):
    ...
    while bytes_recd < total_size:
        ...
        if chunk == b"":
            return "ERROR: From 'download_file' socket connection broken"
        f.write(chunk)
        bytes_recd += len(chunk)

    logging.info(f'Received file from server.')

    output = f"Success downloading file {file_name} server -> client"
    data = {"response":output}
    response = SentMessage(data=data)
    response_bytes = response.create_message()
    print("\n" + output + "\nBucketDrive-> ", end="")
    if s.sendall(response_bytes) == None:
...
def upload_file(bucket_name, file_path):
    ...
    if file_path.is_file():
        ...
        output = "SUCCESS: The file has been uploaded."
        resp = recv_response_ret(s)
        print ("\n" + resp + "\nBucketDrive-> ", end="")
        return output
    else:
        return f"ERROR: File {file_path} does not exist in client"
...
def recv_response_ret(sock):
    rm = ReceivedMessage(is_response=True)
    read_message(rm, sock)
    return rm.data["response"]
